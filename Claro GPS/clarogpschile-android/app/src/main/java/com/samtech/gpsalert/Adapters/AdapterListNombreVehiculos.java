...
public class AdapterListNombreVehiculos extends BaseAdapter /*implements Filterable*/ {
    ...
    /*public Object getVehiculo(int pos){
        if(pos < listaVehiculosFiltrada.size() && pos < vistas.size()) return new VehiculoModel(listaVehiculosFiltrada.get(pos).getIdVehiculo(), listaVehiculosFiltrada.get(pos).getPatenteVehiculo(), vistas.get(pos).etNombrePatente.getText().toString());
