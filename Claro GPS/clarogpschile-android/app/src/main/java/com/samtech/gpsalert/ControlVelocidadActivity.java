...
import com.samtech.gpsalert.Adapters.AdapterListControlVelocidad;
...
public class ControlVelocidadActivity extends ActionBarActivity  implements OnQueryTextListener {
    String Usr, Psw;
    public ListView lv;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(Usr.isEmpty() || Psw.isEmpty()) finish();
            ...
            ControlVelocidadTask cv = new ControlVelocidadTask(Usr,Psw);
            cv.execute((Void) null);
        }
    ...
    public boolean onQueryTextChange(String stringIngresado) {
        AdapterListControlVelocidad adp = (AdapterListControlVelocidad) lv.getAdapter();
    ...
    // Permite cargar la lista de vehiculos para el control de velocidad
    public class ControlVelocidadTask extends AsyncTask<Void, Void, String> {
        private final String urlWS = "http://libs.samtech.cl/movil/ControlDeVelocidad.asp";
        private final String Usr;
        private final String Psw;
        private final String Accion;

        ControlVelocidadTask(String Usuario, String Password) {
            Usr = Usuario;
            Psw = Password;
            Accion = "1";
        }
        ...
        protected void onPostExecute(String Resultado) {
            if(Resultado!=null){
                Log.d("ControlVelActiv", Resultado);
                                ...
                                for(int i=0; i<jsonArr.length();i++){
                                    ...
                                    if (jsonVeh != null) {
                                        if(!jsonVeh.getString("GPS").isEmpty() && !jsonVeh.getString("Patente").isEmpty()) {
                                            ModoEstacionadoModel m = new ModoEstacionadoModel(jsonVeh.getString("GPS"), jsonVeh.getString("Patente"), jsonVeh.getString("Estado"), jsonVeh.getString("Velocidad"), jsonVeh.getString("Nombre"));
                ...
                if(listaVehiculos.size()>0){
                    if(listaVehiculos.size()>1){ // tiene más de un vehiculo, se debe agregar el activar todos
                        ...
                        listaVehiculosAux.add(new ModoEstacionadoModel("0", "Activar Todos", "0", "0", ""));
                        for(int i=0; i<listaVehiculos.size();i++) listaVehiculosAux.add(listaVehiculos.get(i));

                        listaVehiculos = new ArrayList<ModoEstacionadoModel>();
                        listaVehiculos = listaVehiculosAux;
                    }
                    AdapterListControlVelocidad adapter = new AdapterListControlVelocidad(getBaseContext(), Usr, Psw, listaVehiculos);
